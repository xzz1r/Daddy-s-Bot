...
function getActivityPhrases(count) {
  ...
  if (count === 0) {
    return [
      'CERO mensajes. Ni uno. Entras, espías, te pirás y no dejas prueba de vida útil. No es timidez, %N: es parasitismo digital de manual. Consumes lo que otros producen y tu autoestima vive de no arriesgarse a quedar en evidencia, joder.',
      'El contador marca cero, %N. Ni una sílaba. Llevas aquí el tiempo suficiente para que eso ya no sea discreción: es no existir. Un fantasma que se cree interesante por callar, cabrón.',
      'Cero mensajes, %N. El mirón del grupo. Lo lees todo y no aportas una puta mierda. Nadie te echa de menos porque nadie tiene material para recordarte. Ego de misterio, realidad de silla vacía, gilipollas.',
      'Ni un solo mensaje, %N. El grupo no tiene prueba de que existes. Nombre en la lista, espacio ocupado, valor cero. Tu autoestima se esconde detrás del silencio porque el texto te delataría, mierda.',
      'Cero textos, %N. Máximo nivel de gorrón: consumir todo y no dar nada. Estás en cuarenta grupos igual de muerto. La pereza de teclear es el disfraz de no tener nada que valga la pena, coño.',
      'Sin un solo mensaje y ahí sigues pegado, %N. Eso ya no es introversión: es no tener una puta cosa útil que decir y no tener los cojones de largarte. Fantasma-lapa. El ego prefiere invisibilidad a juicio, asco.',
      'Cero mensajes confirmados, %N. Tiempo de sobra para haber soltado algo. No lo hiciste. No es humildad: es vacío. La autoestima te dice que callar te hace profundo; el contador dice que sobras, patético.',
      'El historial dice cero y no miente, %N. Eres el miembro que hincha la lista sin hinchar el chat. Bulto. Decoración. Tu valor aquí es el mismo que el de una silla sin nadie, basura.',
      'Ni respuesta, ni pregunta, ni signo de vida, %N. Cero. Un cero a la izquierda con número de teléfono. La definición del que sobra y encima se cree discreto por no aportar, ridículo.',
      'Cero mensajes, %N. Estar en un grupo de conversación sin conversar nunca. Inutilidad con estética de misterio. El ego lo vende como selectividad; el grupo lo lee como nada, fracasado.',
      'Sin un mensaje, %N. Presente en la lista, ausente en todo lo demás. La forma más barata de pertenecer. Tu autoestima no resiste el riesgo de escribir y quedar expuesto, joder.',
      'No existe un solo mensaje tuyo, %N. En un grupo de comunicación eso dice una cosa: no tienes nada que comunicar y ni la decencia de irte. Basura digital con orgullo de fantasma, cabrón.',
      'Cero. %N ocupa plaza que alguien con algo que decir aprovecharía. Asiento vacío que respira. El inútil de catálogo que sobra y no se entera, gilipollas.',
      'El contador en cero es el resumen de tu valor aquí, %N. No hay subtexto profundo: hay ausencia. El ego prefiere inventar profundidad a arriesgar un texto, mierda.',
      'Cero mensajes, %N. El grupo tomó nota: prescindible con datos. Si desapareces mañana no hay hueco que tapar. Eso debería bajar un poco el auto-concepto, coño.',
    ];
  }

  if (count < 20) {
    return [
      `${c} mensajes en total, %N. Todo lo que has aportado cabe en una pantalla. Decoración de fantasma de medio pelo. La autoestima te dice que menos es más; aquí menos es nada, joder.`,
      `Con ${c} mensajes ocupas una plaza que alguien con voz aprovecharía, %N. Asiento vacío que respira. Sobras y el ego no procesa el dato, cabrón.`,
      `${c} mensajes, %N. Cifra del que no le importa el grupo pero tampoco tiene otro sitio donde fingir pertenencia. Prescindible documentado, gilipollas.`,
      `${c} textos en el historial, %N. Lo justo para confirmar que existes, insuficiente para que a alguien le importe si te borras. Fantasma con wifi, mierda.`,
      `${c} mensajes, %N. El número le dice al grupo cuánto te importa estar: una mierda. Y se nota desde el primer registro, coño.`,
      `Con ${c} mensajes tienes historial de quien entró por error y se quedó por inercia, %N. Sin motivo para aportar. Sin motivo para que te pidan nada. Sobras, asco.`,
      `${c} textos, %N. Lo que dejas al irte es lo mismo que dejas al estar: nada. El fantasma más inútil del ranking de presencia, patético.`,
      `${c} mensajes, %N. Cifra del que no considera que el grupo merezca su tiempo pero no tiene plan B. Don nadie por descarte, basura.`,
      `Con ${c} mensajes eres estadísticamente de lo más prescindible del grupo, %N. No el más silencioso con estilo: el más inútil sin estética, ridículo.`,
      `${c} mensajes, %N. Suficiente para no ser cero del todo, insuficiente para importar. Gris puro. La autoestima busca misterio donde solo hay flojera, fracasado.`,
      `${c} textos, %N. El grupo no te cita porque no hay qué citar. Existir sin dejar marca es tu especialidad, joder.`,
      `Con ${c} mensajes llevas el perfil del que mira de lejos y se cree por encima del ruido. No estás por encima: estás fuera sin gloria, cabrón.`,
      `${c} mensajes, %N. La autoestima te vende selectividad; el contador vende desinterés. El chat se queda con el contador, gilipollas.`,
      `${c} en el historial. %N, ni para ser un buen fantasma: hasta el silencio te queda a medias, mierda.`,
      `${c} mensajes, %N. El ego necesita creer que callar es poder. Aquí callar es evaporarte, coño.`,
    ];
  }

  if (count < 60) {
    return [
      `${c} mensajes, %N. El que lo lee TODO y no aporta NADA. Espectador que consume el trabajo ajeno y se esconde cuando toca poner algo. Parásito con autoestima de crítico, joder.`,
      `Con ${c} mensajes estás en la zona muerta del que está pero no cuenta, %N. Ni fantasma limpio ni parte de nada memorable. Gris prescindible. Sobras a medias y el ego lo niega, cabrón.`,
      `${c} putos textos, %N. Justo debajo del umbral donde alguien empieza a importar. Sigues siendo número de lista, no persona con peso. Don nadie de manual, gilipollas.`,
      `${c} mensajes y el grupo sigue sin saber qué coño pintas, %N. Suficiente para molestar el contador, insuficiente para justificar el hueco, mierda.`,
      `Con ${c} mensajes eres el casi del chat: casi presente, casi útil, casi olvidable. El ego odia el casi; el grupo se acostumbró, coño.`,
      `${c} textos, %N. Actividad de quien quiere figurar sin arriesgar opinión que duela. Cobardía disfrazada de mesura, asco.`,
      `${c} mensajes, %N. El ranking te ve; el hilo no te necesita. Esa brecha debería bajar el auto-concepto, patético.`,
      `Con ${c} mensajes aportas lo justo para no ser borrado y lo poco para ser recordado. Estrategia de mediocre con ego frágil, basura.`,
      `${c} mensajes, %N. Presencia tibia. Ni te extrañan del todo ni te celebran. El limbo del que no se atreve a valer, ridículo.`,
      `${c} textos y cero citas que importen, %N. Hablar sin dejar eco es tu deporte. La autoestima inventa profundidad, el chat mide silencio posterior, fracasado.`,
      `Con ${c} mensajes ocupas el hueco del relleno humano, %N. Hace falta gente así para que el resto brille. No es un cumplido, joder.`,
      `${c} mensajes, %N. El ego lee actividad; el grupo lee irrelevancia funcional. Dos lecturas, una verdad, cabrón.`,
      `${c} en el contador. %N, casi invisible, casi ruidoso, totalmente prescindible en lo que importa, gilipollas.`,
      `${c} mensajes, %N. Suficiente historial para haber dicho una cosa memorable. No llegó. La oportunidad se fue y tu autoestima no se enteró, mierda.`,
      `Con ${c} textos eres el ejemplo del que está sin estar, %N. El chat funciona igual contigo o sin ti. Procesa eso, coño.`,
    ];
  }

  if (count >= 150) {
    return [
      `${c} mensajes, %N. Volumen de sobra y peso a debate. Hablas como si el contador te diera la razón. No te la da: te da altavoz, joder.`,
      `Con ${c} mensajes saturaste el hilo y el ego lo celebró como legado. El grupo a veces solo aguanta, cabrón.`,
      `${c} textos, %N. Actividad de quien confunde cantidad con importancia. La autoestima se financia a crédito de spam, gilipollas.`,
      `${c} mensajes y sigues sin una cita que alguien guarde con orgullo, %N. Ruido con firma. El eco no te debe respeto, mierda.`,
      `Con ${c} mensajes convertiste el chat en tu oficina. Fichas entrada; el resto no votó tu ascenso, coño.`,
      `${c} mensajes, %N. El contador te sube el ego; el contenido a ratos te lo bajaría si miraras de frente, asco.`,
      `${c} textos. %N es inevitable y no siempre bienvenido. Inevitable no es admirado, patético.`,
      `Con ${c} mensajes ya no tienes coartada de perfil bajo. Solo queda el juicio sobre si vales el espacio, basura.`,
      `${c} mensajes, %N. Relevancia por saturación. La saturación cansa más que ilumina, ridículo.`,
      `${c} en el historial. %N, el ego baila con el número; el hilo pide filtro, fracasado.`,
      `Con ${c} mensajes eres clima del grupo. A veces el mal clima. El barómetro no miente, joder.`,
      `${c} textos, %N. Presencia que no pide permiso y a veces no ofrece brillo. Volumen ≠ valor, cabrón.`,
      `${c} mensajes. %N se plantó y no se fue. Territorialidad de quien no tiene otro escenario para el ego, gilipollas.`,
      `Con ${c} mensajes el ranking de presencia te corona de cartón. La corona no pesa talento, mierda.`,
      `${c} mensajes, %N. El grupo te tiene fichado. Fichado no es querido. Procesa la diferencia, coño.`,
      `${c} textos y la autoestima leyendo 'imprescindible' donde decía 'presente', %N. Error de traducción, asco.`,
      `Con ${c} mensajes cargaste el hilo a hombros o lo aplastaste. A veces no se distingue, patético.`,
      `${c} mensajes, %N. Saturación con nombre. El chat no aplaude el exceso por educación, basura.`,
      `${c} en el contador. %N, presencia de quien no sabe irse a tiempo. El timing también es valor, ridículo.`,
      `Con ${c} mensajes convertiste cantidad en identidad. Pobre identidad si solo es métrica, fracasado.`,
      `${c} mensajes, %N. El ego se alimenta del contador mejor que del respeto ajeno, joder.`,
      `${c} textos. %N es parte del mobiliario y del problema de ruido, cabrón.`,
      `Con ${c} mensajes ya eres estadística gruesa. Las estadísticas no tienen aura de genio, gilipollas.`,
      `${c} mensajes, %N. Importante por no callar. Callar a tiempo también construye valor, mierda.`,
      `${c} textos y cero freno, %N. La autoestima confunde insistencia con liderazgo, coño.`,
      `Con ${c} mensajes el hilo te reconoce al vuelo y a veces suspira. Reconocer no es admirar, asco.`,
      `${c} mensajes, %N. El número te sostiene cuando el resto no cita. Muleta cara, patético.`,
      `${c} en el pecho del contador. %N, medalla de hojalata si el contenido no acompaña, basura.`,
      `Con ${c} mensajes ocupas el centro sin que te hayan pedido el micrófono, ridículo.`,
      `${c} mensajes, %N. Relevancia forzada a martillazos de texto. El público es cautivo, no fan, fracasado.`,
    ];
  }

  return [
      `${c} mensajes, %N. Estás en la franja donde ya no eres fantasma y todavía no eres peso. El limbo del ego inseguro, joder.`,
      `Con ${c} mensajes aportas lo suficiente para que no te borren y lo poco para que te nombran con entusiasmo, cabrón.`,
      `${c} textos, %N. Actividad de quien prueba suerte sin jugarse el tipo. La autoestima evita el riesgo del texto memorable, gilipollas.`,
      `${c} mensajes y el grupo aún negocia si importas. Esa negociación debería humillar un poco, mierda.`,
      `Con ${c} mensajes eres el relleno intermedio: ni heroína del hilo ni cero. El gris con wifi, coño.`,
      `${c} textos, %N. Historial de quien podría haber dejado marca y prefirió no arriesgar el ego, asco.`,
      `${c} mensajes. %N aparece, comenta, se esfuma del significado. Presente sin consecuencia, patético.`,
      `Con ${c} mensajes tienes material para un perfil y no para una leyenda. El ego elige leyenda igual, basura.`,
      `${c} mensajes, %N. Suficiente para ser visto, insuficiente para ser necesario. La diferencia duele si eres honesto, ridículo.`,
      `${c} textos y cero momentos que alguien repita, %N. Hablar sin eco es tu zona de confort, fracasado.`,
      `Con ${c} mensajes el contador te da carnet de miembro y el hilo te da carnet de extra, joder.`,
      `${c} mensajes, %N. El ego quiere más; el contenido no empuja. Fricción visible, cabrón.`,
      `${c} en el historial. %N, mitad presencia, mitad relleno. El chat lo nota sin decirlo, gilipollas.`,
      `${c} mensajes, %N. Actividad correcta de quien no quiere quedar en evidencia ni brillar, mierda.`,
      `Con ${c} textos estás a un paso de importar y a un paso de diluirte. El ego no elige: se queda en medio, coño.`,
  ];
}
...
const OWNER_ROAST = [
  ...
  '%N se cree el centro del chat y el contador a veces le hace el favor. asco Y el grupo lo tiene claro, joder.',
