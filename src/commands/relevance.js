...
let PARASITO = [
  ...
  '%N, %C mensajes. Parasitas el contador sin dejar una sola frase útil. Y el ranking no miente, asco con el eco del fail todavía sonando.',
  ...
  '%N, %C mensajes. Irrelevancia medible: el bot no inventa, cuenta. Y el ranking no miente, pringado y el sistema cierra sin discusión.',
  ...
  '%N, %C mensajes. Mirón de primera: el título no se disputa. Y el ranking no miente, patético delante del ranking y de la cara.',
  ...
  '%N, %C mensajes. Parasito documentado por %C mensajes de nada útil. Y el ranking no miente, asco sin consuelo de manual de autoayuda.',
  '%N, %C mensajes. El contador te ve: el hilo no te necesita. Y el ranking no miente, cutre con el número hablando solo.',
  '%N, %C mensajes. Fantasma de los que leen el drama y nunca lo firman. Y el ranking no miente, pringado.',
  '%N, %C mensajes. Relevancia cero con evidencia numérica. Y el ranking no miente, fracasado y el archivo queda cerrado hoy.',
  '%N, %C mensajes. Ocupas espacio de miembro: el espacio no se defiende solo. Y el ranking no miente, joder.',
  '%N, %C mensajes. Parasitas notificaciones ajenas como deporte. Y el ranking no miente, mierda y duele de lo previsible.',
  '%N, %C mensajes. El ranking de aporte te deja donde mereces: abajo. Y el ranking no miente, coño sin derecho a bis ni a matiz.',
  '%N, %C mensajes. Silencio que no es estrategia: es vacío. Y el ranking no miente, cabrón con el chat de testigo obligado.',
  '%N, %C mensajes. Mirón con historial: el bot traduce historial a veredicto. El grupo lo nota cada día, gilipollas.',
  '%N, %C mensajes. Irrelevante de forma estable: no es un mal mes. Y el ranking no miente, patético punto final del parte de hoy.',
  '%N, %C mensajes. El grupo no se cae sin ti: se aligera. Y el ranking no miente, ridículo y no hay segunda lectura útil.',
  '%N, %C mensajes. Parasito del contador: sumas presencia y restas sustancia. El grupo lo nota cada día, basura.',
  '%N, %C mensajes. Fantasma oficial del ranking de relevancia. Y el ranking no miente, desperdicio en alta definición de group chat.',
  '%N, %C mensajes. %C mensajes y cero eco útil en el hilo. Y el ranking no miente, asco como manda el ranking sin filtro.',
  ...
  '%N, %C mensajes. Irrelevancia con sello del comando. Y el ranking no miente, fracasado sin descuento emocional posible.',
  '%N, %C mensajes. El mirón del grupo tiene nombre: el tuyo. Y el ranking no miente, joder y el contador no miente nunca.',
  '%N, %C mensajes. Silencio de relleno: el relleno se expulsa cuando toca. Y el ranking no miente, mierda.',
  '%N, %C mensajes. Relevancia bajo tierra: %C mensajes de prueba. Y el ranking no miente, coño delante de quien miraba de verdad.',
  '%N, %C mensajes. Parasito sin gracia: ni el silencio tiene estilo. Y el ranking no miente, cabrón con firma legible en el historial.',
  '%N, %C mensajes. El hilo no te abre hueco: tú no abriste ninguno. Y el ranking no miente, gilipollas.',
  '%N, %C mensajes. Fantasma con contador: la peor combinación. Y el ranking no miente, patético y basta de cuento para hoy.',
  '%N, %C mensajes. Irrelevante y documentado: no hay debate. Y el ranking no miente, ridículo al natural, sin barniz de consuelo.',
  ...
  '%N, %C mensajes. Mirón crónico: el oficio no tiene jubilación aquí. Y el ranking no miente, asco que es como duele de verdad aquí.',
  '%N, %C mensajes. Relevancia inexistente con %C mensajes de coartada. Y el ranking no miente, cutre y el resto del hilo es ruido.',
  '%N, %C mensajes. El chat no nota tu ausencia: nota tu falta de aporte. Y el ranking no miente, pringado.',
  '%N, %C mensajes. Parasito del hilo: diagnóstico cerrado. Y el ranking no miente, fracasado sin modo avión que lo oculte.',
  '%N, %C mensajes. Silencio que no aporta misterio: aporta hueco. Y el ranking no miente, joder con el bot firmando el veredicto.',
  '%N, %C mensajes. Irrelevancia medible en %C mensajes. Y el ranking no miente, mierda y el grupo ya cambió de tema.',
  '%N, %C mensajes. Fantasma que el bot se cansa de no oír. Y el ranking no miente, coño en el único idioma del ranking.',
  '%N, %C mensajes. Parasitas y el contador no te absuelve. Y el ranking no miente, cabrón sin apelación ni letra pequeña.',
  '%N, %C mensajes. Relevancia en números rojos de sustancia. Y el ranking no miente, gilipollas con el saldo escrito a la vista.',
  ...
  '%N, %C mensajes. Parasito sin narrativa heroica: solo el número. Y el ranking no miente, basura y no hace falta ampliar el parte.',
  ...
  '%N, %C mensajes. Relevancia cero: %C mensajes no la inventan. Y el ranking no miente, fracasado en el momento más incómodo del chat.',
  '%N, %C mensajes. Mirón profesional con carnet del contador. Y el ranking no miente, joder sin que nadie pida repetición.',
  '%N, %C mensajes. Silencio de quien no aporta: el bot no lo confunde con elegancia. El grupo lo nota cada día, mierda.',
  '%N, %C mensajes. Irrelevante y estable: el peor pronóstico. Y el ranking no miente, coño con el eco del fail todavía sonando.',
  '%N, %C mensajes. Parasito del grupo documentado sin anestesia. Y el ranking no miente, cabrón y el sistema cierra sin discusión.',
  '%N, %C mensajes. El ranking de aporte te deja en el sótano con %C mensajes de testigo. El grupo lo nota cada día, gilipollas.',
  '%N, %C mensajes. Relevancia bajo mínimos legales del chat. Y el ranking no miente, ridículo delante del ranking y de la cara.',
  '%N, %C mensajes. Parasitas el hilo y el hilo no te devuelve eco. Y el ranking no miente, basura sin consuelo de manual de autoayuda.',
  '%N, %C mensajes. Mirón: el título se gana no escribiendo nada útil. Y el ranking no miente, desperdicio.',
  '%N, %C mensajes. Irrelevancia con %C mensajes de prueba cargada. Y el ranking no miente, asco con el número hablando solo.',
  '%N, %C mensajes. El chat no te cita porque no hay material. Y el ranking no miente, cutre y el archivo queda cerrado hoy.',
  '%N, %C mensajes. Parasito sin estilo: ni el vacío tiene gracia. Y el ranking no miente, pringado y duele de lo previsible.',
  '%N, %C mensajes. Silencio que el bot traduce a irrelevancia. Y el ranking no miente, fracasado sin derecho a bis ni a matiz.',
  ...
  '%N, %C mensajes. Parasitas el contador de miembros sin el de aporte. Y el ranking no miente, coño con el chat de testigo obligado.',
  '%N, %C mensajes. Mirón del drama ajeno: el drama no te nombra. Y el ranking no miente, cabrón punto final del parte de hoy.',
  '%N, %C mensajes. Irrelevante de forma que ya no sorprende. Y el ranking no miente, gilipollas y no hay segunda lectura útil.',
  ...
  '%N, %C mensajes. Fantasma que no deja frase: solo rastro de lectura. Y el ranking no miente, asco en alta definición de group chat.',
  '%N, %C mensajes. Parasitas y el bot no ofrece indulgencia. Y el ranking no miente, cutre como manda el ranking sin filtro.',
  '%N, %C mensajes. Mirón con %C mensajes: el número no te salva. Y el ranking no miente, pringado sin descuento emocional posible.',
  '%N, %C mensajes. Irrelevancia documentada: punto final del tramo. Y el ranking no miente, fracasado.',
  '%N, %C mensajes. El hilo no te necesita: el ranking lo certifica. Y el ranking no miente, joder y el contador no miente nunca.',
  '%N, %C mensajes. Parasito sin misterio: el vacío se ve. Y el ranking no miente, mierda delante de quien miraba de verdad.',
  '%N, %C mensajes. Relevancia cero con evidencia de %C mensajes. Y el ranking no miente, coño con firma legible en el historial.',
  '%N, %C mensajes. Fantasma del chat: el oficio no se discute. Y el ranking no miente, cabrón y basta de cuento para hoy.',
  '%N, %C mensajes. Mirón crónico: el contador es el carnet. Y el ranking no miente, patético al natural, sin barniz de consuelo.',
  ...
  '%N, %C mensajes. Relevancia bajo tierra: no hay ascensor. Y el ranking no miente, asco que es como duele de verdad aquí.',
  '%N, %C mensajes. Fantasma con nick: el pack del que no pinta nada. Y el ranking no miente, cutre y el resto del hilo es ruido.',
  '%N, %C mensajes. Parasitas el promedio y no subes ninguno que importe. Y el ranking no miente, pringado.',
  '%N, %C mensajes. Mirón: el drama se escribe sin tu firma. Y el ranking no miente, fracasado sin modo avión que lo oculte.',
  '%N, %C mensajes. Irrelevancia medible y estable. Y el ranking no miente, joder con el bot firmando el veredicto.',
  '%N, %C mensajes. El chat sigue igual sin tu aporte: porque no hay aporte. Y el ranking no miente, mierda.',
  '%N, %C mensajes. Parasito documentado por el comando de relevancia. Y el ranking no miente, coño y el grupo ya cambió de tema.',
  '%N, %C mensajes. Relevancia inexistente: %C mensajes no crean sustancia. Y el ranking no miente, cabrón.',
  '%N, %C mensajes. Fantasma oficial del sótano del ranking. Y el ranking no miente, gilipollas en el único idioma del ranking.',
  '%N, %C mensajes. Parasitas el grupo: el grupo no te parasita de vuelta. Y el ranking no miente, patético.',
  '%N, %C mensajes. Mirón con historial vacío de frases útiles. Y el ranking no miente, ridículo sin apelación ni letra pequeña.',
  '%N, %C mensajes. Irrelevante de fábrica: este mensaje es el sello. Y el ranking no miente, basura con el saldo escrito a la vista.',
  '%N, %C mensajes. Silencio de quien ocupa sitio: el sitio se puede liberar. El grupo lo nota cada día, desperdicio.',
  '%N, %C mensajes. Parasito sin gracia ni misterio: solo el número %C. Y el ranking no miente, asco y no hace falta ampliar el parte.',
