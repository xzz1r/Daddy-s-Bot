...
function addStickerMeta(webpBuffer) {
  // Add EXIF metadata for WhatsApp sticker pack info
  const pack = config.sticker.pack;
  const author = config.sticker.author;

  const json = JSON.stringify({ 'sticker-pack-id': 'socialbot', 'sticker-pack-name': pack, 'sticker-pack-publisher': author });
