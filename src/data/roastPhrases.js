...
// Pools de !roast. La lógica vive en commands/roast.js.
...
const COMBINED_INACTIVE = [
  ...
  '%N, eres de esa gente que se pone una bio patética para tapar lo poco que vale. Y aun así no escribes nada porque el silencio es lo único que te protege del ridículo absoluto que te mereces, cabrón.',
  ...
  '%N, eres el gilipoyas que tiene una bio que da vergüenza y la desfachatez de seguir en el grupo sin aportar absolutamente nada. Existes como el olor a humedad: molesto, sin valor y sin remedio.',
...
const COMBINED_ACTIVE = [
  ...
  '%N, eres de esa gente que se pone una bio patética para tapar lo poco que vale. Y encima hablas como si alguien te hubiera pedido que participaras. Nadie te pidió nada, gilipoyas. Cierra el puto pico.',
  ...
  '%N, eres el gilipoyas que tiene una bio que da vergüenza y aun así sigue mandando mensajes como si alguien estuviera esperando lo que tiene que decir. Nadie espera nada tuyo, puto. Nunca lo ha esperado.',
  ...
  '%N, eres ruido con forma de persona. La bio miente y tú entre medias insistes en participar como si alguien hubiera pedido tu opinión alguna vez. No la pidió nadie, gilipoyas. Nunca.',
  ...
  'Escribes como un pobre gilipoyas gritando en un cuarto vacío, %N. La bio pretende y el grupo pasa de ti. Mucho esfuerzo para confirmar lo único constante en ti: que hagas lo que hagas, sigues sobrando.',
...
const BIO_EMPTY = [
  'Sin bio. El único espacio del planeta donde decides cómo quieres que te vean y lo dejaste en blanco. Eso no es misterio, es que no hay una sola cosa dentro de ti que merezca una puta frase, gilipoyas.',
  ...
  'La bio vacía es la única decisión que has tomado en tu vida que tiene sentido. Mostraste lo que hay dentro: nada. Primera vez que eres completamente honesto con el mundo, gilipoyas.',
...
const BIO_FULL = [
  ...
  '%N, la descripción que pusiste para impresionar consiguió lo contrario con una precisión que ni tú calculaste, gilipoyas. Todo tu puto talento invertido en hacer el ridículo sin enterarte. El don nadie en plenitud, exhibiéndose gratis.',
...
module.exports = { HEADERS, CLOSERS, COMBINED_INACTIVE, COMBINED_ACTIVE, NAME_ONLY, BIO_EMPTY, BIO_FULL, OWNER_ROAST };
